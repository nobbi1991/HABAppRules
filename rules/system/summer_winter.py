...
class SummerWinter(HABApp.Rule):
    ...
    def __init__(self, outside_temperature_name: str, summer_name: str, persistence_service: str = None, days: int = 7, temperature_threshold: float = 16, last_check_name: str = None) -> None:
        ...
        self._summer_item = HABApp.openhab.items.SwitchItem.get_item(summer_name)
        self._last_check_item = HABApp.openhab.items.DatetimeItem.get_item(summer_name) if last_check_name else None
    ...
    def __get_weighted_mean(self, days_in_past: int) -> float:
        ...
        :return: the weighted mean according the formula in docstring
    ...
    def __is_summer(self) -> bool:
        ...
        if not len(values) > self._days * 0.5:
    ...
    def _cb_update_summer(self) -> None:
        ...
        try:
            ...
            LOGGER.exception("Could not get is_summer")
            return
        ...
        # send state
        if self._summer_item.value != target_value:
            self._summer_item.oh_send_command(target_value)
            LOGGER.info(f"Summer changed to {target_value}")

        # update last update item at every call
        if self._last_check_item:
            self._last_check_item.oh_send_command(datetime.datetime.now())
