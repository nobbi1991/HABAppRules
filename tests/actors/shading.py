...
class TestShadingBase(tests.helper.test_case_base.TestCaseBase):
	...
	def test_cb_sleep_state(self):
		...
		test_cases = [
			TestCase(habapp_rules.system.SleepState.AWAKE, False, True),
			...
			TestCase(habapp_rules.system.SleepState.POST_SLEEPING, False, False),
...
class TestSetNight(tests.helper.test_case_base.TestCaseBase):
	...
	def test_init_without_elevation(self):
		"""Test init without elevation."""
		with unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.soon") as run_soon_mock, \
				unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.on_sun_dawn") as run_dawn_mock, \
				unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.on_sun_dusk") as run_dusk_mock:
			rule = habapp_rules.actors.shading.SetNight("Unittest_Night")

		run_soon_mock.assert_not_called()
		run_dusk_mock.assert_called_once_with(rule._set_night, target_value="ON")
		run_dawn_mock.assert_called_once_with(rule._set_night, target_value="OFF")

	def test_init_with_elevation(self):
		"""Test init without elevation."""
		TestCase = collections.namedtuple("TestCase", "elevation_value, called_with")
		...
		with unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.soon") as run_soon_mock, \
				...
				unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.on_sun_dusk") as run_dusk_mock:
			for test_case in test_cases:
				run_soon_mock.reset_mock()
				run_dusk_mock.reset_mock()
				run_dawn_mock.reset_mock()
				tests.helper.oh_item.set_state("Unittest_Elevation", test_case.elevation_value)

				rule = habapp_rules.actors.shading.SetNight("Unittest_Night", "Unittest_Elevation")

				run_dusk_mock.assert_called_once_with(rule._set_night, target_value="ON")
				run_dawn_mock.assert_called_once_with(rule._set_night, target_value="OFF")

				if test_case.called_with is None:
					run_soon_mock.assert_not_called()
				else:
					run_soon_mock.assert_called_once_with(rule._set_night, target_value=test_case.called_with)

	def test_set_night(self):
		"""Test _set_night."""
		with unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.soon"), \
				unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.on_sun_dawn"), \
				unittest.mock.patch("HABApp.rule.scheduler.habappschedulerview.HABAppSchedulerView.on_sun_dusk"):
			rule = habapp_rules.actors.shading.SetNight("Unittest_Night")

		tests.helper.oh_item.assert_value("Unittest_Night", None)

		rule._set_night("ON")
		tests.helper.oh_item.assert_value("Unittest_Night", "ON")

		rule._set_night("OFF")
		tests.helper.oh_item.assert_value("Unittest_Night", "OFF")
