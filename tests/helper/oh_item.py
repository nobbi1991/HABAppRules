...
_MOCKED_ITEM_NAMES = []
StateTypes = str | int | float | datetime.datetime


def add_mock_item(item_type: type[HABApp.openhab.items.OpenhabItem], name: str, initial_value: str | int | float | None = None) -> None:
    """Add a mock item.
...
def set_state(item_name: str, value: StateTypes | None) -> None:
    ...
    try:
        ...
        print(f"Could not set '{value}' to '{item_name}'")  # noqa: T201


def send_command(item_name: str, new_value: StateTypes, old_value: StateTypes = None) -> None:
    """Replace of send_command for unit-tests.
...
def assert_value(item_name: str, value: StateTypes, message: str | None = None) -> None:
    """Assert if item has correct state.
