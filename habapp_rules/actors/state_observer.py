...
class StateObserverRollerShutter(_StateObserverBase):
    ...
    def send_command(self, value: float) -> None:
        """Send command to the observed item.

        :param value: Value to send to the item
        """
        if not isinstance(value, int | float):
            raise TypeError(f"The given value is not supported for StateObserverDimmer: {value}")
...
class StateObserverNumber(_StateObserverBase):
    ...
    def send_command(self, value: int | float) -> None:
        ...
        """
        if not isinstance(value, int | float):
            raise TypeError(f"The given value is not supported for StateObserverNumber: {value}")
        self._value = value
        self._item.oh_send_command(value)


class StateObserverSlat(StateObserverNumber):
    """Observer for slat items of shading."""
