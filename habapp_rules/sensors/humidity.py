...
from HABApp.openhab.events.event_filters import ItemStateUpdatedEventFilter
...
from habapp_rules.sensors.config.humidity import HumiditySwitchConfig

LOGGER = logging.getLogger(__name__)


class HumiditySwitch(StateMachineRule):
    ...
    def __init__(self, config: HumiditySwitchConfig) -> None:
        """Init humidity rule.
        ...
        """
        self._config = config
        StateMachineRule.__init__(self, self._config.items.state, self._config.items.humidity.name)
        ...
        self._set_state_timeout("on_Extended", self._config.parameter.extended_time)

        # register callbacks
        self._config.items.humidity.listen_event(self._cb_humidity, ItemStateUpdatedEventFilter())
    ...
    def _cb_humidity(self, event: ItemStateUpdatedEvent) -> None:
        ...
        if self._check_high_humidity(event.value):
            self.trigger("high_humidity_start")
        else:
            self.trigger("high_humidity_end")
